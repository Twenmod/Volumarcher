...
namespace
{
	...
	void KbmUpdate()
	{
		...
		{
			if (GameCore::g_mouseLocked == true)
			{
				s_Mouse->SetCooperativeLevel(GameCore::g_hWnd, DISCL_FOREGROUND | DISCL_EXCLUSIVE);
				s_Mouse->Acquire();
...
#define SET_BUTTON_VALUE(InputEnum, GameInputMask) \
	...
	if (ERROR_SUCCESS == XInputGetState(0, &newInputState))
	{
		...
		s_Analogs[kAnalogLeftStickY] = FilterAnalogInput(newInputState.Gamepad.sThumbLY,
		                                                 XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE);
		s_Analogs[kAnalogRightStickX] = FilterAnalogInput(newInputState.Gamepad.sThumbRX,
		                                                  XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);
		s_Analogs[kAnalogRightStickY] = FilterAnalogInput(newInputState.Gamepad.sThumbRY,
		                                                  XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);
	}
#else
